...
function TransactionStatement(props) {
  const { billing, totalPrice, totalMileage, buyerBusiness } = props;
  ...
  const totalQuantity = useMemo(() => {
    const detail = billing?.flatMap(item => item.orderDetails);
  ...
  useEffect(() => {
    setIsMileageUse(billing.mileagePointUsage > 0);
  }, [billing]);
      ...
      >
        ...
        <tbody>
          ...
          >
            ...
            >
              {/* ₩2.767.600 &lt;VAT 포함&gt; */}₩
              {formatCostPrint(Math.round(totalPrice) - totalMileage)} &lt;VAT
              포함&gt;
            </td>
          </tr>
          <tr
          ...
          {billing.map((item, index) => {
            return item.orderDetails.map((dt, dtIndex) => {
              const unitPrice = Math.round(
                (dt?.unitPrice ?? 0) + (dt?.productOptions[0]?.price ?? 0),
              );
              totalVat += unitPrice * dt.quantity * 0.1;
              totalUnitPrice += unitPrice * dt.quantity;
              return (
                <>
                  <tr
                    key={dt?.product?.id}
          ...
          >
            ...
            >
              {/* 2.516.000 */}
              {formatCostPrint(totalUnitPrice)}
            </td>
            <td
              className="xl106"
              style={{ borderLeft: 'none', padding: '3px' }}
            ...
            >
              {/* 251.600 */}
              {formatCostPrint(Math.round(totalVat))}
            </td>
            <td
              className="xl107"
              style={{ borderTop: 'none', borderLeft: 'none', padding: '3px' }}
            ...
            >
              {/* 2.767.600 */}
              {formatCostPrint(Math.round(totalPrice) - totalMileage)}
            </td>
          </tr>
