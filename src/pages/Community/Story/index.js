...
import { storyTabs, storyTabsUrlHash } from '../../../constants';
import AllStory from './All';
...
function Page() {
  ...
  const renderContent = () => {
    switch (storyTab) {
      case storyTabs.ALL:
        return <AllStory />;
      case storyTabs.COMPANY_NEWS:
        return <AllStory categoryType="COMPANY_NEWS" />;
      case storyTabs.NEW_PRODUCT:
        return <AllStory categoryType="NEW_PRODUCT" />;
      case storyTabs.EVENTS:
        return <AllStory categoryType="EVENTS" />;
      case storyTabs.REPORT:
        return <AllStory categoryType="REPORT" />;
      default:
        return <AllStory />;
    }
  };

  const handleChangeCurrentTab = (tab, hashURL) => {
  ...
  useEffect(() => {
    switch (location?.hash.replace('#', '')) {
      case storyTabsUrlHash.ALL:
        setStoryTab(storyTabs.ALL);
        break;
      case storyTabsUrlHash.COMPANY_NEWS:
        setStoryTab(storyTabs.COMPANY_NEWS);
        break;
      case storyTabsUrlHash.NEW_PRODUCT:
        setStoryTab(storyTabs.NEW_PRODUCT);
        break;
      case storyTabsUrlHash.EVENTS:
        setStoryTab(storyTabs.EVENTS);
        break;
      case storyTabsUrlHash.REPORT:
        setStoryTab(storyTabs.REPORT);
        break;
      default:
        setStoryTab(storyTabs.ALL);
        break;
    ...
    <Container className="my-page-container">
      ...
      >
        <div className="story-title">
          <p className="my-page-title">라오나크 이야기</p>
          <p className="story-second-title">새로운 소식을 안내드립니다.</p>
        </div>
        <div className="tab-button-container">
          <div className="tab-button">
            <button
              onClick={() =>
                handleChangeCurrentTab(storyTabs.ALL, storyTabsUrlHash.ALL)
              }
              className={storyTab === storyTabs.ALL ? 'active' : ''}
            >
              전체
            </button>
            <button
              onClick={() =>
                handleChangeCurrentTab(
                  storyTabs.COMPANY_NEWS,
                  storyTabsUrlHash.COMPANY_NEWS,
                )
              }
              className={storyTab === storyTabs.COMPANY_NEWS ? 'active' : ''}
            >
              회사소식
            </button>
            <button
              onClick={() =>
                handleChangeCurrentTab(
                  storyTabs.NEW_PRODUCT,
                  storyTabsUrlHash.NEW_PRODUCT,
                )
              }
              className={storyTab === storyTabs.NEW_PRODUCT ? 'active' : ''}
            >
              신제품 출시
            </button>
            <button
              onClick={() =>
                handleChangeCurrentTab(
                  storyTabs.EVENTS,
                  storyTabsUrlHash.EVENTS,
                )
              }
              className={storyTab === storyTabs.EVENTS ? 'active' : ''}
            >
              행사안내
            </button>
            <button
              onClick={() =>
                handleChangeCurrentTab(
                  storyTabs.REPORT,
                  storyTabsUrlHash.REPORT,
                )
              }
              className={storyTab === storyTabs.REPORT ? 'active' : ''}
            >
              보도자료
            </button>
          </div>
