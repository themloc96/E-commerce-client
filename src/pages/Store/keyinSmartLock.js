...
import AcceptMethods from './components/AcceptMethods';
...
function Page() {
  ...
  const handleChangeApp = (event, screen) => {
    ...
    if (checked) {
      ...
      setListOption(prev => [...prev, value]);
    } else {
      // remove unchecked value from the list
      setCurrentApps(prev => prev.filter(pr => pr !== value));
      setListOption(prev => prev.filter(pr => pr !== value));
    }
  };
